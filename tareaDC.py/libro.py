class Book:
    def _init_(self, Titulo, Autor , ISBN , Publicacion , Status):
        self.__autor = Autor
    ...
    def feedback (self , Titulo:str):
        if Titulo in self.__titulo:
            ...
            self._comentario.update({self._titulo : comentario})
    ...
    def renw_info (self, Titulo):
        if Titulo in self.__duet:
            ...
            if len(values)>1:
                return self._titulo , self.autor , self.isbn , self.publicacion ,values, self._status
            else:
                return self._titulo , self.autor , self.isbn , self.publicacion , self._status 